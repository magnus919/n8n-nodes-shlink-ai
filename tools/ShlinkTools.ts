...
export async function getShortUrlDetails(
  ...
  try {
    ...
    const visitCount = (data as any).visitsSummary?.total || (data as any).visitsCount || 0;
    ...
    return {
      ...
      data: data,
      metadata: {
        human_summary: `Short URL ${(data as any).shortCode} points to ${(data as any).longUrl} with ${visitCount} visits`,
        actionable_fields: {
          short_url: (data as any).shortUrl,
          short_code: (data as any).shortCode,
          long_url: (data as any).longUrl,
    ...
    return {
      ...
      metadata: {
        ...
        actionable_fields: {
          ...
          items_per_page: (data as any).pagination?.itemsPerPage,
          total_pages: (data as any).pagination?.pagesCount
        },
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        data: data,
        metadata: {
          human_summary: `Failed to get visits for ${params.shortCode}: ${(data as any).detail || 'Unknown error'}`,
          actionable_fields: { short_code: params.shortCode },
    ...
    const totalVisits = (data as any).pagination?.totalItems || (data as any).data?.length || 0;
    const recentVisits = (data as any).data?.slice(0, 5) || [];
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        data: data,
        metadata: {
          human_summary: `Failed to edit ${params.shortCode}: ${(data as any).detail || 'Unknown error'}`,
          actionable_fields: { short_code: params.shortCode },
...
export async function deleteShortUrl(
  ...
  try {
    ...
    return {
      ...
      data: data,
      metadata: {
        human_summary: `Failed to delete ${params.shortCode}: ${(data as any).detail || 'Unknown error'}`,
        actionable_fields: { short_code: params.shortCode },
